...
defmodule TypeSafeSDK.Examples.Evaluation.CLI do
  ...
  def run(args) do
    ...
    {opts, rest, invalid} = OptionParser.parse(args, strict: [split: :string, dataset: :string,
      output: :string, policy: :string, model: :string, sweep: :boolean,
      confidence: :float, urgency: :float, max_auto_error: :float, max_concurrency: :integer])
    if rest != [] or invalid != [], do: raise(ArgumentError, "unknown evaluation CLI option")
    split = Keyword.get(opts, :split, "development")
    unless split in ["development", "held-out"], do: raise(ArgumentError, "split must be development or held-out")
    ...
    if Keyword.has_key?(opts, :policy) and (sweep? or Enum.any?([:confidence, :urgency], &Keyword.has_key?(opts, &1))),
      do: raise(ArgumentError, "a frozen policy cannot be combined with --sweep or threshold overrides")
    ...
    declared_policy = Eval.validate_policy!(%{
    ...
    {policy, selection_error} = cond do
      frozen -> {frozen["policy"], nil}
      sweep? ->
        try do
    ...
    report = %{"sdk_version" => TypeSafeSDK.version(), "split" => split,
      "requested_model" => model, "policy" => policy, "metrics" => metrics,
      "selection_error" => selection_error, "frozen_model_mismatch" => !!model_mismatch,
      "threshold_sweep" => sweep, "records" => records,
      "dataset_kind" => "synthetic illustrative dataset; not benchmark evidence"}
    output = Keyword.get(opts, :output, "tmp/triage-#{split}.json")
    write!(output, report)
    if split == "development" and not is_nil(metrics) and metrics["successes"] > 0 and metrics["failures"] == 0 do
      case metrics["models"] do
        [observed_model] ->
          write!(Path.rootname(output) <> ".policy.json", %{"source_split" => "development",
            "model" => observed_model, "policy" => policy, "source_report" => output})
        _ -> IO.warn("No frozen policy written: development responses used different model versions")
      end
    end
