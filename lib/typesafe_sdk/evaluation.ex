defmodule TypeSafeSDK.Evaluation do
  @moduledoc false
  alias TypeSafeSDK.{Client, Error, JSON, Prepared, RetryPolicy, SemanticResponse, SystemOneResponse, Telemetry}
  alias TypeSafeSDK.Question.Validation
  @options [:model, :timeout, :timeout_ms, :retry, :extra_body, :extra_headers,
            :probability_tolerance, :telemetry_metadata]
  ...
  def run(%Client{} = client, state, questions, opts \\ []) do
    Telemetry.span(metadata(client, questions, opts), fn ->
      ...
      result = with :ok <- validate_options(opts),
           {:ok, prepared} <- Prepared.new(questions),
           {:ok, state} <- JSON.normalize(state, ["state"]),
           {:ok, extra} <- extra_body(Keyword.get(opts, :extra_body)),
           {:ok, response} <- execute(client, state, prepared, extra, opts),
           {:ok, response} <- SemanticResponse.enrich(response, prepared,
             Keyword.get(opts, :probability_tolerance, 0.02)) do
        {:ok, response}
      end
  ...
  defp execute(client, state, prepared, extra, opts) do
    body = Map.merge(extra, %{"state" => state, "questions" => prepared.encoded,
      "model" => Keyword.get(opts, :model) || client.default_model})
    case TypeSafeSDK.Generated.SystemOne.create(client, body, Keyword.drop(opts, @semantic_options)) do
  ...
  defp optional_model(model) when is_binary(model) do
    if String.valid?(model) and String.trim(model) != "", do: :ok,
  ...
  defp timeout(value, key, _) when is_number(value) and value > 0 and value < 1.0e12 do
    ...
    if value >= minimum, do: :ok, else: invalid(["options", key], "must resolve to at least 1 millisecond")
  end
  ...
  defp probability_tolerance(_), do: invalid(["options", "probability_tolerance"], "must be in [0, 0.1]")
  ...
  defp retry(value) when is_list(value) do
    cond do
      not Keyword.keyword?(value) -> invalid(["options", "retry"], "must be a keyword list")
    ...
    with {:ok, pairs} <- JSON.keyed_pairs(value, path) do
      Enum.reduce_while(pairs, {:ok, MapSet.new()}, fn {key, value}, {:ok, seen} ->
        ...
        cond do
          ...
          true -> {:cont, {:ok, MapSet.put(seen, normalized)}}
        end
      end)
      |> case do
        {:ok, _} -> :ok
  ...
  defp metadata(client, questions, opts) do
    ...
    count = case questions do
      %Prepared{count: count} -> count
      q when is_map(q) and not is_struct(q) -> map_size(q)
      q when is_list(q) -> safe_count(q)
      _ -> nil
    end
    caller = Keyword.get(opts, :telemetry_metadata, %{})
    model = Keyword.get(opts, :model) || client.default_model
    %{operation: :evaluate, requested_model: if(is_binary(model), do: model, else: nil),
      question_count: count, caller: if(is_map(caller) and not is_struct(caller), do: caller, else: %{})}
  end
