defmodule TypeSafeSDK.BatchTest do
  ...
  test "preparation and shared-option failures occur before spawning requests" do
    ...
    for opts <- [[max_concurrency: 0], [ordered: :yes], [task_timeout_ms: :infinity],
      [on_error: :ignore], [attempt_timeout_ms: 5, timeout_ms: 3]] do
  ...
  test "concurrency is bounded and early halt kills remaining owned tasks" do
    owner = self()
    client = Test.client() |> Test.stub_callback(fn request ->
    ...
    consumer = Task.async(fn ->
      client |> TypeSafeSDK.evaluate_stream(Stream.iterate(0, &(&1 + 1)), questions(), max_concurrency: 2)
      |> Enum.take(1)
    end)
  ...
  test "per-state timeout preserves index and never reports the raw state as an exit reason" do
    client = Test.client() |> Test.stub_callback(fn _request ->
      receive do
        :never -> {:answers, [q: {:noul, 0.9}]}
      end
    end)
    assert [{:error, error}] = TypeSafeSDK.evaluate_many(client, ["private-state"], questions(), task_timeout_ms: 30)
  ...
  test "unordered results retain association with input indexes" do
    ...
    consumer = Task.async(fn ->
      ...
      |> Enum.map(fn result -> send(owner, {:result, result}); result end)
    end)
  ...
  test "caller death cleans the supervisor and its tasks" do
    ...
    consumer = spawn(fn -> TypeSafeSDK.evaluate_many(client, [0, 1], questions(), max_concurrency: 2) end)
  ...
  test "on_error raise uses the normalized SDK error, and attempts use millisecond option" do
    ...
    assert_raise Error, fn -> TypeSafeSDK.evaluate_many(client, ["x"], questions(), on_error: :raise) end
    client = Test.stub(client, q: {:noul, 0.8})
    assert [{:ok, _}] = TypeSafeSDK.evaluate_many(client, ["x"], questions(), attempt_timeout_ms: 777)
  ...
  test "ordered prefetch and completed-result buffering cannot run past the declared window" do
    owner = self()
    states = Stream.map(0..100, fn index -> send(owner, {:pulled, index}); index end)
    client = Test.client() |> Test.stub_callback(fn request ->
      index = Jason.decode!(request.body)["state"]
      if index == 0 do
    ...
    consumer = Task.async(fn ->
      TypeSafeSDK.evaluate_stream(client, states, questions(),
        max_concurrency: 2, max_pending: 3, ordered: true)
      |> Enum.take(1)
    end)
