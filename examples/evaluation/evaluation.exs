defmodule TypeSafeSDK.Examples.Evaluation do
  ...
  def questions do
    TypeSafeSDK.prepare!(
      department:
        TypeSafeSDK.choice("Which team owns the primary support request?",
      ...
      urgent:
        TypeSafeSDK.noul("Does this require urgent human attention?",
  ...
  def run(client, rows, opts \\ []) do
    ...
    results =
      TypeSafeSDK.evaluate_many(client, Enum.map(rows, & &1["state"]), prepared,
  ...
  def record(row, {:ok, response}) do
    with {:ok, %TypeSafeSDK.ChoiceAnswer{} = department} <-
           TypeSafeSDK.Response.fetch(response, :department),
