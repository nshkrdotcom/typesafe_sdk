defmodule TypeSafeSDK.SemanticResponseTest do
  ...
  defp questions do
    TypeSafeSDK.prepare!(urgent: TypeSafeSDK.noul("Urgent?"),
      team: TypeSafeSDK.choice("Team?", billing: "Payments", support: "Product"),
      level: TypeSafeSDK.score("Severity?", [{"Low", "Cosmetic"}, {"High", "Blocking"}]))
  end
  defp body do
    %{"model" => "jev-test", "usage" => %{"input_tokens" => 3, "output_tokens" => 2},
      "answers" => %{
        "urgent" => %{"type" => "noul", "noul" => 0.9},
        "team" => %{"type" => "choice", "choice" => "billing", "confidence" => 0.4,
          "probabilities" => %{"billing" => 0.8, "support" => 0.2}},
        "level" => %{"type" => "score", "score" => 0.7, "confidence" => 0.8,
  ...
  test "known missing, extra and wrong-type answers are not silently accepted" do
    assert {:error, %Error{path: ["answers", "team"]}} = decode(update_in(body()["answers"], &Map.delete(&1, "team")))
    assert {:error, %Error{path: ["answers", "extra"]}} = decode(put_in(body(), ["answers", "extra"], %{"type" => "noul", "noul" => 1}))
    assert {:error, %Error{path: ["answers", "team", "type"]}} = decode(put_in(body(), ["answers", "team"], %{"type" => "noul", "noul" => 1}))
  end

  test "unknown future answers remain raw and do not fail known-answer completeness" do
    raw = body() |> put_in(["answers", "team"], %{"type" => "future", "payload" => true})
  ...
  test "wire scalars reject out-of-range probabilities and negative usage" do
    for {path, value} <- [
      ...
      {["usage", "input_tokens"], -1}, {["usage", "output_tokens"], 0.5}
    ] do
      assert {:error, %Error{type: :response_validation}} = SystemOneResponse.decode(put_in(body(), path, value))
    end
  end

  test "normalizing numeric distribution keys cannot overwrite colliding entries" do
    invalid = put_in(body(), ["answers", "level", "probabilities"], %{"0" => 0.2, "00" => 0.1, "1" => 0.7})
    assert {:error, %Error{field_path: "answers.level.probabilities"}} = SystemOneResponse.decode(invalid)
  end
  ...
  test "Choice ranking uses explicit caller tie order and margin is not confidence" do
    answer = %TypeSafeSDK.ChoiceAnswer{choice: :z, confidence: 0.9,
      probabilities: %{a: 0.5, z: 0.5}, option_order: [:z, :a]}
  ...
  test "rounded expectation can be the least likely level" do
    answer = %TypeSafeSDK.ScoreAnswer{score: 1.0, confidence: 0.4,
