defmodule TypeSafeSDK.Telemetry do
  ...
  def span(metadata, fun) do
    ...
    try do
      ...
      emit(:stop, Map.put(measurements, :duration, System.monotonic_time() - start),
        Map.merge(metadata, outcome))
      result
    catch
      kind, reason ->
        emit(:exception, %{duration: System.monotonic_time() - start},
  ...
  def cancelled(index, reason) when reason in [:timeout, :task_exit] do
    :telemetry.execute([:typesafe_sdk, :batch, :cancelled], %{count: 1},
      %{operation: :evaluate, batch_index: index, outcome: reason})
  end
  ...
  defp outcome({:ok, %SystemOneResponse{} = response}) do
    ...
    {measurements, %{outcome: :ok, model: response.model, status: status,
      request_id: response.request_id, retries: response.retries}}
  end
  defp outcome({:error, %Error{} = error}) do
    {%{}, %{outcome: :error, error_type: error.type, status: error.status,
      request_id: error.request_id, retries: Map.get(error.details, :retries)}}
  end
