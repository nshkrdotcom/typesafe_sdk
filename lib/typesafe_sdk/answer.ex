defmodule TypeSafeSDK.Answer do
  ...
  def confidence(%NoulAnswer{} = answer), do: TypeSafeSDK.Answer.Noul.confidence(answer)
  ...
  defdelegate yes?(answer), to: TypeSafeSDK.Answer.Noul
  defdelegate yes?(answer, threshold), to: TypeSafeSDK.Answer.Noul
  ...
  def gate(answer, opts) do
    with :ok <- TypeSafeSDK.Question.Validation.options(opts, [:act, :review]),
      ...
      _ -> raise ArgumentError, "gate requires 0 <= review <= act <= 1, with both thresholds explicit"
    end
  end
end
