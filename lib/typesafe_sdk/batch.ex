defmodule TypeSafeSDK.Batch do
  ...
  @batch_options [:max_concurrency, :max_pending, :ordered, :on_error, :task_timeout_ms, :attempt_timeout_ms]
  ...
  def stream(client, states, questions, opts \\ []) do
    ...
    Stream.resource(
      fn ->
        {:ok, lifecycle} = TypeSafeSDK.Batch.Lifecycle.start(self(), task_opts[:max_concurrency])
        supervisor = TypeSafeSDK.Batch.Lifecycle.supervisor(lifecycle)
  ...
  defp prepare!(questions, opts) do
    ...
    check!(is_integer(max_concurrency) and max_concurrency in 1..1024, "max_concurrency", "must be in 1..1024")
    max_pending = if is_nil(max_pending), do: max_concurrency * 4, else: max_pending
    check!(is_integer(max_pending) and max_pending >= max_concurrency and max_pending <= 10_000,
      "max_pending", "must be at least max_concurrency and at most 10000")
    check!(is_integer(task_timeout) and task_timeout > 0, "task_timeout_ms", "must be a positive integer")
    ...
    {prepared, request, [max_concurrency: max_concurrency, ordered: ordered,
      timeout: task_timeout, on_timeout: :kill_task, zip_input_on_exit: true,
      shutdown: :brutal_kill], on_error, max_pending}
  end
  ...
  defp evaluate_input(client, {state, index}, prepared, request_opts) do
    ...
    result = try do
      ...
      kind, _reason ->
        ...
        {:error, %Error{type: :task_exit, message: "Batch evaluation worker failed",
          details: %{scope: :batch, kind: kind,
  ...
  defp attempt_timeout!(request, value) do
    ...
    check!(not Keyword.has_key?(request, :timeout_ms) and not Keyword.has_key?(request, :timeout),
      "attempt_timeout_ms", "cannot be combined with timeout or timeout_ms")
  ...
  defp result({:exit, {{_state, index}, reason}}, policy) do
    ...
    error = %Error{type: type, message: "Batch evaluation #{index} #{type}",
